...
def complete(price):
	try:
		with open(f"intermarche/history.dat", "r") as f:
			data = f.read()
			data = data.split(", ")
	except:
		data = []
	...
	with open(f"intermarche/history.dat", "w") as f:
		f.write(str(text))

def main():
	current_week = getWeek()
	try:
		with open(f"intermarche/week.dat", "r") as f:
			week = f.read()
	except:
		week = current_week
		get()
	...
	if int(week) != current_week:
		get()
	with open(f"intermarche/week.dat", "w") as f:
